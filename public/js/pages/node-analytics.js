// NodakMesh Dashboard - Node Analytics Page

import { getNodeById } from "../api.js";
...
function getRoleBadge(role) {
  ...
  const badgeClasses = {
    repeater: "badge-cyan",
    room_server: "badge-amber",
    client: "badge-green",
  };
  const label = roleLabels[role] || role || "Node";
  const cls = badgeClasses[role] || "badge-cyan";
...
function buildHeader(container, node) {
  ...
  roleBadge.innerHTML = getRoleBadge(node.role);
...
export function mount(container, params = {}) {
  ...
  if (!nodeId) {
    container.innerHTML = '<div class="empty-state"><div class="empty-title">No node ID specified</div></div>';
    return;
  }
...
async function loadNodeData(container, nodeId) {
  ...
  try {
    ...
    container.innerHTML = `
      <div class="empty-state">
        <div class="empty-title">Node Not Found</div>
        <div class="empty-message">Could not load data for node "${nodeId}".</div>
      </div>
    `;
    return;
  }
  ...
  if (node == null) {
    container.innerHTML = `
      <div class="empty-state">
        <div class="empty-title">Node Not Found</div>
        <div class="empty-message">No data available for node "${nodeId}".</div>
      </div>
    `;
    return;
  }
